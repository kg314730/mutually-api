...
router.get('/search', async (req, res)=>{
    try{
        const users = await User.find({current_company:req.query.company.toUpperCase()});
        if(users.length ==0){
            throw new Error(`No results found for ${req.query.company}`);
        }
        const results = users.map(user=>{
            const {_id, name, email, current_company, current_experience, current_position, profile_picture, ...rest} = user.toJSON();
            return {_id, name, email, current_company, current_experience, current_position, profile_picture};
        })
